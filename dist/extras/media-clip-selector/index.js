...
class MediaClipSelector extends window.HTMLElement {
  ...
  handleClick(evt) {
    ...
    if (this.isTimestampInBounds(timestampForClick)) {
      this.dispatchEvent(new window.CustomEvent(MediaUIEvents.MEDIA_SEEK_REQUEST, {composed: true, bubbles: true, detail: timestampForClick}));
    }
  }
  mediaCurrentTimeSet(time) {
    ...
    if (!this.mediaPaused) {
      ...
      if (this.mediaCurrentTime < startTime || this.mediaCurrentTime > endTime) {
        this.dispatchEvent(new window.CustomEvent(MediaUIEvents.MEDIA_SEEK_REQUEST, {composed: true, bubbles: true, detail: startTime}));
      }
    }
  }
  ...
  enableThumbnails() {
    ...
    const trackMouse = () => {
      mouseMoveHandler = (evt) => {
        ...
        this.dispatchEvent(new window.CustomEvent(MediaUIEvents.MEDIA_PREVIEW_REQUEST, {composed: true, bubbles: true, detail: mousePercent * duration}));
      };
