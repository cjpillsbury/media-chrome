...
class MediaCaptionsButton extends MediaChromeButton {
  ...
  handleClick(_e) {
    ...
    if (ccIsOn) {
      const captionsShowingStr = this.getAttribute(MediaUIAttributes.MEDIA_CAPTIONS_SHOWING);
      if (captionsShowingStr) {
        const evt = new window.CustomEvent(MediaUIEvents.MEDIA_DISABLE_CAPTIONS_REQUEST, {composed: true, bubbles: true, detail: captionsShowingStr});
        this.dispatchEvent(evt);
      }
      const subtitlesShowingStr = this.getAttribute(MediaUIAttributes.MEDIA_SUBTITLES_SHOWING);
      if (subtitlesShowingStr && !this.hasAttribute("no-subtitles-fallback")) {
        const evt = new window.CustomEvent(MediaUIEvents.MEDIA_DISABLE_SUBTITLES_REQUEST, {composed: true, bubbles: true, detail: subtitlesShowingStr});
        this.dispatchEvent(evt);
      }
    } else {
      const [ccTrackStr] = (_b = splitTextTracksStr((_a = this.getAttribute(MediaUIAttributes.MEDIA_CAPTIONS_LIST)) != null ? _a : "")) != null ? _b : [];
      if (ccTrackStr) {
        const evt = new window.CustomEvent(MediaUIEvents.MEDIA_SHOW_CAPTIONS_REQUEST, {composed: true, bubbles: true, detail: ccTrackStr});
        this.dispatchEvent(evt);
      } else if (!this.hasAttribute("no-subtitles-fallback")) {
        const [subTrackStr] = (_d = splitTextTracksStr((_c = this.getAttribute(MediaUIAttributes.MEDIA_SUBTITLES_LIST)) != null ? _c : "")) != null ? _d : [];
        if (subTrackStr) {
          const evt = new window.CustomEvent(MediaUIEvents.MEDIA_SHOW_SUBTITLES_REQUEST, {composed: true, bubbles: true, detail: subTrackStr});
          this.dispatchEvent(evt);
        }
      } else {
